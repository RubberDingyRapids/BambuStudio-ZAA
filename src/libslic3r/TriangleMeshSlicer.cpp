...
static CoplanarEdges coplanar_edges(const indexed_triangle_set &mesh, const std::vector<Vec3i> &face_edge_ids,
                                    const Transform3d &trafo)
{
    ...
    int num_edges = 0;
    for (const Vec3i &edge_ids : face_edge_ids)
    ...
    auto face_plane = [&mesh, &face_planes, &face_plane_computed, &transformed_vertex](int face_idx) -> const FacePlane& {
        if (! face_plane_computed[face_idx]) {
            const Vec3i &face = mesh.indices[face_idx];
