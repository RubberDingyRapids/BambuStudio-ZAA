...
// Based on original work by Alexander Thor licensed under the GPLv3:
...
static bool safc_nearly_equal(double a, double b)
{
...
{
    try {
        for (const auto& line : config.small_area_infill_flow_compensation_model.values) {
            ...
            if (std::getline(iss, value_str, ',')) {
                try {
                    ...
                    std::stringstream ss;
                    ss << "Small Area Flow Compensation: Error parsing data point in small area infill compensation model:"
                       << line << std::endl;
        ...
        for (size_t i = 0; i < eLengths.size(); i++) {
            if (i == 0) {
                if (!safc_nearly_equal(eLengths[i], 0.0)) {
        ...
        if (!flowComps.empty() && !safc_nearly_equal(flowComps.back(), 1.0)) {
            throw Slic3r::InvalidArgument(
