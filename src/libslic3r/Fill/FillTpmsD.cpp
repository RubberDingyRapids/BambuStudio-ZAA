...
// TPMS "Schwarz D" infill.
// Implicit surface: sin(x)*sin(y)*sin(z) - cos(x)*cos(y)*cos(z) = 0
// Solved analytically for one family of iso-lines per Z slice, the same way
// FillGyroid solves the gyroid surface: substituting u=x-y, v=x+y turns the
// equation into a*cos(u) - b*cos(v) = 0 with a=sin(z)-cos(z), b=sin(z)+cos(z),
// which is a simple 1D wave in (u,v) space that gets tiled and sheared back
// into (x,y).
    ...
    Polylines                       &polylines_out)
{
    ...
    // Distance between the waves in scaled coordinates.
